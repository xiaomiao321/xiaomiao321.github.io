...
7.5 pb  04.07.25 Optimization on customizer settings
...
BottomMountingHoles_X_Distance = 30;
// Distance between the mounting holes in Y direction
BottomMountingHoles_y_Distance = 50;
// Move all mounting holes in X direction
BottomMountingHolesOffset_X    = 0;
// Move all mounting holes in Y direction
BottomMountingHolesOffset_Y    = 0;

/* [Top cover hole settings] */
// Activate customizable holes on the top cover
ShowTopCoverHoles         = false;
// Number of holes in X direction (length direction)
TopCoverHoles_X_Count     = 2;     //[1:1:10]
// Number of holes in Y direction (width direction)
TopCoverHoles_Y_Count     = 2;     //[1:1:10]
// Diameter of the holes on top cover
TopCoverHoleDiameter      = 5;     //[1:0.1:50]
// Spacing between holes in X direction
TopCoverHoleSpacing_X     = 20;    //[5:1:200]
// Spacing between holes in Y direction
TopCoverHoleSpacing_Y     = 20;    //[5:1:200]
// Offset of hole pattern in X direction from center
TopCoverHoleOffset_X      = 0;     //[-100:1:100]
// Offset of hole pattern in Y direction from center
TopCoverHoleOffset_Y      = 0;     //[-100:1:100]
...
module BodyTop () {
    if (ShowTop)
    {
        difference(){
            ...
            // Top cover holes
            if (ShowTopCoverHoles)
            {
                for (ix = [-(TopCoverHoles_X_Count-1)/2 : (TopCoverHoles_X_Count-1)/2])
                {
                    for (iy = [-(TopCoverHoles_Y_Count-1)/2 : (TopCoverHoles_Y_Count-1)/2])
                    {
                        translate([
                            ix * TopCoverHoleSpacing_X + TopCoverHoleOffset_X,
                            iy * TopCoverHoleSpacing_Y + TopCoverHoleOffset_Y,
                            -0.01
                        ]) 
                        cylinder(
                            h = CutFromTop + 0.02,
                            d = TopCoverHoleDiameter,
                            center = false
                        );
                    }
                }
